...
def extract_real_content(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # The marker that appears in the new template
    marker = 'Houdini / Tool</span>'
    
    # Check if this marker appears twice (which indicates corruption)
    start_indices = [m.start() for m in re.finditer(re.escape(marker), content)]
    
    if len(start_indices) < 2:
        print(f"Warning: {file_path} does not seem to have duplicated structure. Parsing as best effort from the first marker.")
        # Fallback: assume it's roughly correct or single?
        # If single, we might want to check if it's already "good" but we want to be safe.
        # Let's assume we take the LAST marker always, because if it's single, last is first.
        # If it's double, last is the inner one.
        pass

    last_marker_index = start_indices[-1]
    
    # Substring from the last marker
    sub = content[last_marker_index:]
    
    # Next, we expect the <h1>Title</h1> to be the first H1 in this substring
    # EXCEPT, the regex <h1>(.*?)</h1> excludes attributes.
    # In the corrupted file, the inner structure *should* have <h1>Title</h1> without attributes (inserted by my previous script).
    
    title_match = re.search(r'<h1>(.*?)</h1>', sub, re.DOTALL)
    if not title_match:
        # Fallback if title extraction fails
        return "Tool", "<p>Error extracting content</p>"
        
    title = title_match.group(1).strip()
    
    # The content starts AFTER this H1
    content_start = title_match.end()
    
    # The content ends at the next </article>
    ...
    if article_end_match:
        ...
        clean_content = sub[content_start:content_end].strip()
    else:
        # If no </article> found (weird), take everything? No, that's dangerous.
        clean_content = "<p>Error parsing content bounds</p>"
...
def main():
    for file_name in FILES_TO_UPDATE:
        ...
        try:
            title, content_html = extract_real_content(path)
            
            # Construct new HTML
            # Note: We must insert the Title H1 manually because we excluded it from clean_content
            html_content = f"<h1>{title}</h1>\n{content_html}"
            
            new_html = TEMPLATE_TOP.replace('__TITLE__', title) + html_content + TEMPLATE_BOTTOM.replace('__SCRIPT__', COPY_SCRIPT)
