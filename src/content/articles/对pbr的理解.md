...
- 非金属F0固定为4%
金属的反射率值，与绝缘体的反射颜色一起存储在固有色贴图中

1. 固有色贴图中的非金属区域表示反照颜色，金属区域表示的是反射率数值。
1. 固有色不应当含有光照信息，微观遮蔽（micro-occlusion）信息除外。
1. 暗部数值不应低于30 sRGB(宽松范围) - 50 sRGB(严格范围)。
1. 亮部数值不应高于240 sRGB。
1. 金属的反射率较高，有70-100%之间的镜面反射，映射到 sRGB 为180-255
### Specular
